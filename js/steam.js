...
window.Steam = (function () {
  ...
  async function fetchWishlist() {
    ...
    // Helper to parse raw Steam wishlist JSON into game objects
    ...
    // 1. Try loading the static file generated by the GitHub Action
    try {
      console.log('[Steam] Trying local wishlist-data.json...');
      const res = await fetch('./wishlist-data.json', { signal: AbortSignal.timeout(5000) });
      if (res.ok) {
        const data = await res.json();
        const games = parseWishlistData(data);
        if (statusText) statusText.textContent = 'Connected';
        if (badge) badge.classList.add('connected');
        console.log(`[Steam] Loaded ${games.length} games from local file.`);
        return games;
      }
    } catch (err) {
      console.warn('[Steam] Local file not available:', err.message);
    }
    ...
    for (const makeProxy of PROXIES) {
      try {
        ...
        if (statusText) statusText.textContent = 'Connected';
        if (badge) badge.classList.add('connected');
        console.log(`[Steam] Loaded ${games.length} games via proxy.`);
        return games;
      } catch (err) {
    ...
    console.error('[Steam] All sources failed:', lastErr?.message, '— using demo data.');
    if (statusText) statusText.textContent = 'Demo mode';
    if (badge) badge.classList.add('error');
    return normalizeDemoGames();
  }
